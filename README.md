# Sobre el proyecto Matlab

* Este repositorio será un recopilatorio de los códigos que he realizado en Matlab como tareas/proyectos de las diferentes actividades académicas de mi vida Universitaria.
* La mayoría de los proyectos tienen relación a mi carreara como Ingeniero en Electrónica y Telecomunicaciones. Básicamente señales, codificaciones, protocolos, electrónica básica, no obstante, también he realizado proyectos típicos en el aprendizaje de programación como calculadoras, facturación y ejercicios propuestos.
* Subiré todos los códigos que dispongo poco a poco agregando en esta introducción una tabla con una consideración de la dificultad de desarrollo/entendimiento (opinión propia). Adicionalmente, trataré de mejorar y/o optimizar los códigos en el proceso, por lo que también subiré una consideración del porcentaje de desarrollo.

## Lista de programas

|Programa| Dificultad | Fase Desarrollo | Observaciones |
|:----:|:----:|:----:|:----:|
| Analisis Voz Espetral FFT | ![Dificultad][d1] | ![BarraProgreso][b100] | ------ |
| CalculadoraVoz | ![Dificultad][d2] | ![BarraProgreso][b100] | Se puede mejorar. |
| Conexion Arduino | ------ | ------ | Libreria desarrollada por STAFF de Mathworks |
| LempelZiv | ![Dificultad][d3] | ![BarraProgreso][b90] | Bug al descomprimir |
| Metodos Numericos | **EN REVISIÓN** | ![BarraProgreso][b50] | ------ |
| Modulacion AM | **EN REVISIÓN** | ![BarraProgreso][b70] | ------ |
| Nodo Erlang | **EN REVISIÓN** | ![BarraProgreso][b20] | ------ |
| Movimiento Brazo Mecanico | **EN REVISIÓN** | ![BarraProgreso][b20] | ------ |
| Sistema Inventario.Facturacion | **EN REVISIÓN** | ![BarraProgreso][b10] | ------ |
| Otros | **EN REVISIÓN** | ------ | Programas varios |

## Contacto

Ing. Guillermo Andrade

Correo: guilloan92@gmail.com


[b10]: https://github.com/gingin92/Matlab/blob/master/config/images/10.png "Barra 10%"
[b20]: https://github.com/gingin92/Matlab/blob/master/config/images/20.png "Barra 20%"
[b30]: https://github.com/gingin92/Matlab/blob/master/config/images/30.png "Barra 30%"
[b40]: https://github.com/gingin92/Matlab/blob/master/config/images/40.png "Barra 40%"
[b50]: https://github.com/gingin92/Matlab/blob/master/config/images/50.png "Barra 50%"
[b60]: https://github.com/gingin92/Matlab/blob/master/config/images/60.png "Barra 60%"
[b70]: https://github.com/gingin92/Matlab/blob/master/config/images/70.png "Barra 70%"
[b80]: https://github.com/gingin92/Matlab/blob/master/config/images/80.png "Barra 80%"
[b90]: https://github.com/gingin92/Matlab/blob/master/config/images/90.png "Barra 90%"
[b100]: https://github.com/gingin92/Matlab/blob/master/config/images/100.png "Barra 100%"

[d1]: https://github.com/gingin92/Matlab/blob/master/config/images/dif1.png "Dificiltad 1"
[d2]: https://github.com/gingin92/Matlab/blob/master/config/images/dif2.png "Dificiltad 2"
[d3]: https://github.com/gingin92/Matlab/blob/master/config/images/dif3.png "Dificiltad 3"
[d4]: https://github.com/gingin92/Matlab/blob/master/config/images/dif4.png "Dificiltad 4"
[d5]: https://github.com/gingin92/Matlab/blob/master/config/images/dif5.png "Dificiltad 5"